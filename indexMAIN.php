...
<?php
    ...
    if(isset($_GET['dhaka2020'])){
    
        $districts = $db->select('select * from dhaka2020');
    }else{
    ...
    if(!isset($_COOKIE[$cookie_name])) {
        ...
        if(isset($_POST['submit_vote'])){
            ...
            foreach ($topic as $key => $value) {
                ...
                if(isset($_GET['dhaka2020'])){
             	$db->insert("INSERT INTO `newVoteTable` (`id`, `name`, `phone`, `email`, `election_area`, `topic`, `comment`, `user_id`, `election_year`) VALUES (NULL, '$name', '$phone', '$email', '$election_area', '$tpc', '$comment', '$_COOKIE[$cookie_name]', 'dhaka2020')");
                }else{
...
<?php
    ...
    if(isset($_GET['dhaka2020'])){
        ...
        $queryResult = $db->select('select * from newVoteTable WHERE election_year="dhaka2020"');
        
    }else{
    
        $queryResult = $db->select('select * from vote');
                ...
                <div class="col-six vote_list_box" id="vot-tricker">
                    ...
                    <div class="card-body card_design">
                       ...
                       <?php if($data['election_area'] != NULL) { echo '<a href="#" class="btn_custom info">From <span class="voteNo">'.$data['election_area'].'</span></a>'; } ?>
                        ...
                        <?php if($data['election_year'] != NULL) { echo '<a href="#" class="btn_custom info">From <span class="voteNo">'.$data['election_year'].'</span></a>'; } ?>
                    </div>
                     <?php } ?>
                    </ul >
                </div>
